"""
Functions which are only to be used in menu.py
"""
...
def make_menu_recursive(src_path, folder_name, menu_name):
    ...
    if menu_name in [each.name() for each in MenuItem.items()]:
        MenuItem.removeItem(menu_name)
        print("Removed Nuke Menu as it was already present")
...
def make_gizmo_menu_recurcively(src_path, folder_name, menu_name, icon):
    ...
    if menu_name in [each.name() for each in node_menu.items()]:
        node_menu.removeItem(menu_name)
        print("Removed Node Menu as it was already present")
