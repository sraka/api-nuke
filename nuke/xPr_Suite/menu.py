...
if os.path.exists(platform_pref_path()):  # Preferences Available
    ...
    if pref.read_from_write is True:
        from PrPy.ReadFromWrite import read_from_write
        top.addCommand("Read from Write", "read_from_write()", "Shift+R", icon="Read_from_Write_icon_v01.png")
    ...
    if pref.delete_thumbs_tmp is True:
        ...
        top.addCommand("Delete Thumbs.db and .tmp Read(s)", "delete_thumbs_tmp()", "Ctrl+Alt+H",
                       icon="Delete_Thumbs_tmp_v01.png")
    if pref.open_read_folder is True:
        from PrPy.OpenFolder import open_read_file
        top.addCommand("Open Read Folder", "open_read_file()", "E", icon="Open_Folder_v01.png")
    if pref.open_nuke_file_folder is True:
        from PrPy.OpenFolder import open_nuke_script
        top.addCommand("Open Nuke File Folder", "open_nuke_script()", icon="Open_Folder_v01.png")
    if pref.open_dot_nuke is True:
        from PrPy.OpenFolder import open_dot_nuke
        top.addCommand("Open .nuke Folder", "open_dot_nuke()", "Shift+D", icon="Open_Folder_v01.png")
    if pref.open_pp_folder is True:
        from PrPy.OpenFolder import open_specific_plugin_path
        top.addCommand("Open Specific PLUGIN_PATH Folder", "open_specific_plugin_path()", "",
                       icon="Open_Folder_v01.png")
    top.addSeparator()
    ...
    top.addCommand("Open .nuke Folder", "open_dot_nuke()", "Shift+D", icon="Open_Folder_v01.png")
    from PrPy.OpenFolder import open_specific_plugin_path
    top.addCommand("Open Specific PLUGIN_PATH Folder", "open_specific_plugin_path()", "", icon="Open_Folder_v01.png")
    top.addSeparator()
    top.addCommand("Preferences", "pref_launch()", "", icon="Preferences_v01.png")
    if platform.system() == "Windows":
        top.addCommand("Setup Pr_Suite Nuke Renders", "renders_setup()", icon="Preferences_v01.png")
